...
<div className="flex sm:hidden min-h-screen w-full bg-[#F5F7FF]">
  {mobileViewSection === "sidebar" ? (
    <div className="w-full">
      ...
      <div className="bg-[#273e8e] h-[300px] text-white p-4 px-5">
        <div className="flex items-center gap-2 mb-8">
          <ChevronLeft/>
          <h2 className="text-lg font-medium absolute left-56 text-white">More</h2>
        </div>

        <div className="flex items-center gap-4 mt-8">
          <div className="w-16 h-16 rounded-full bg-gray-300 overflow-hidden">
            <img src={assets.userImage} alt="User" className="w-full h-full object-cover" />
          </div>
          <div>
            <p className="text-base font-semibold">Qamardeen Abdul malik</p>
            <p className="text-xs text-gray-300">qamardeenoladimeji@gmail.com</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white p-4 -mt-20 rounded-2xl border-[2px] border-gray-300 space-y-2">
        <div onClick={() => handleMobileSectionChange("editProfile")}>
          <SidebarOption colorBg="bg-[#273E8E]" icon={LuUserRound} label="Profile Settings" />
        </div>
        <SidebarOption colorBg="bg-[#8E2778]" icon={PiShoppingCartSimple} label="My Orders" />
        <div onClick={() => handleMobileSectionChange("transactionHistory")}>
          <SidebarOption colorBg="bg-[#8E2727]" icon={PiNotepadBold} label="Transaction History" />
        </div>
        <div onClick={() => handleMobileSectionChange("kycDetails")}>
          <SidebarOption colorBg="bg-[#278E5B]" image={assets.kwc} label="KYC Details" />
        </div>
        <div onClick={() => handleMobileSectionChange("referrals")}>
          <SidebarOption colorBg="bg-[#5C278E]" icon={PiUsersBold} label="Referrals" />
        </div>
        <div onClick={() => handleMobileSectionChange("support")}>
          <SidebarOption colorBg="bg-[#27608E]" image={assets.support} label="Support" />
        </div>
        <div onClick={() => { setShowPasswordPopup(true); setMobileViewSection("content"); }}>
          <SidebarOption colorBg="bg-[#8E2778]" icon={TbPassword} label="Change Password" />
        </div>
      </div>
    </div>
  ) : (
    <div className="w-full p-4">
      {/* Back Button */}
      <div className="flex items-center gap-2 mb-4 text-[#273E8E]" onClick={() => setMobileViewSection("sidebar")}>
        <ChevronLeft />
